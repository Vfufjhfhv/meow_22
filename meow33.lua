...
local DEFAULT_CPS = 360
local MAX_BURST_PER_FRAME = 12

local spam_accum = 0
local real_cps_counter = 0
local last_cps_check = tick()
getgenv().RealCPS = 0

local function get_current_cps()
    if getgenv().ManualSpamCPSEnabled and getgenv().ManualSpamCPS then
        return getgenv().ManualSpamCPS
    end
    return DEFAULT_CPS
end

local function start_optimized_spam()
    ...
    Connections_Manager['Manual Spam UI'] = RunService.PreRender:Connect(function(dt)
        -- Lọc dt bất thường khi giật lag
        if dt > 0.1 then 
            spam_accum = 0 
            return
        end

        local target_cps = get_current_cps()
        spam_accum = spam_accum + (target_cps * dt)
        
        local executed_this_frame = 0
        while spam_accum >= 1 and executed_this_frame < MAX_BURST_PER_FRAME do
            if System and System.parry and System.parry.execute then
                System.parry.execute()
            elseif parry_remote then
                parry_remote:FireServer()
            end

            spam_accum = spam_accum - 1
            executed_this_frame = executed_this_frame + 1
            real_cps_counter = real_cps_counter + 1
        end

        if spam_accum > MAX_BURST_PER_FRAME then
            spam_accum = 0
        end

        local now = tick()
        if now - last_cps_check >= 1 then
            getgenv().RealCPS = real_cps_counter
            real_cps_counter = 0
            last_cps_check = now
        end
    end)
...
local function stop_optimized_spam()
    if Connections_Manager['Manual Spam UI'] then
        Connections_Manager['Manual Spam UI']:Disconnect()
...
local manual_spam_module = SpamTab:create_module({
    ...
    callback = function(state)
        ...
        if state then
            ...
            if System and System.__properties and System.__properties.__manual_spam_ui and System.__properties.__manual_spam_ui.gui then
                pcall(function()
...
manual_spam_cps_slider = manual_spam_module:create_slider({
    ...
    callback = function(value)
        ...
        if warn_manual_spam_cps then warn_manual_spam_cps(value) end
    end
})
...
-- ============================================================
-- TARGET PLAYER LOGIC & HOOK SYSTEM
-- ============================================================
local Players = cloneref and cloneref(game:GetService("Players")) or game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

getgenv()._TargetParryConnections = getgenv()._TargetParryConnections or {}
for key, conn in pairs(getgenv()._TargetParryConnections) do
    if typeof(conn) == "RBXScriptConnection" and conn.Connected then
        conn:Disconnect()
    end
end
table.clear(getgenv()._TargetParryConnections)

getgenv().TargetParryEnabled = getgenv().TargetParryEnabled or false
getgenv().TargetUsername = getgenv().TargetUsername or ""
if getgenv().TargetParryNotify == nil then getgenv().TargetParryNotify = true end
getgenv()._LastTargetPlayer = getgenv()._LastTargetPlayer or nil

local function findValidPlayer(nameStr)
    ...
    for _, p in ipairs(allPlayers) do
        if string.sub(string.lower(p.Name), 1, #query) == query then 
            return p 
        end
    end
...
local function getTargetCharacter(playerObj)
    if not playerObj then return nil end
    local aliveFolder = workspace:FindFirstChild("Alive")
    if aliveFolder then
        local char = aliveFolder:FindFirstChild(playerObj.Name)
        if char then return char end
    end
    return playerObj.Character
end

getgenv()._GetTargetParryCFrame = function()
    if getgenv().TargetParryEnabled and getgenv().TargetUsername ~= "" then
        local targetObj = findValidPlayer(getgenv().TargetUsername)
        if targetObj then
            local targetChar = getTargetCharacter(targetObj)
            local targetPrimary = targetChar and targetChar.PrimaryPart
            local localChar = LocalPlayer.Character
            local localRoot = localChar and (localChar:FindFirstChild("HumanoidRootPart") or localChar.PrimaryPart)

            if targetPrimary and localRoot then
                return CFrame.new(localRoot.Position, targetPrimary.Position)
            end
        end
    end
    return nil
...
local function setTargetPlayer(rawName)
    local targetObj = findValidPlayer(rawName)
    
    if targetObj then
        getgenv().TargetUsername = targetObj.Name
        if getgenv().TargetParryNotify and getgenv()._LastTargetPlayer ~= targetObj then
            pcall(function()
                Library.SendNotification({ 
                    title = "Target Player", 
                    text = "Locked: " .. targetObj.DisplayName .. " (@" .. targetObj.Name .. ")", 
                    duration = 2 
                })
            end)
        end
        getgenv()._LastTargetPlayer = targetObj
    else
        getgenv().TargetUsername = rawName or ""
        getgenv()._LastTargetPlayer = nil
        if getgenv().TargetParryNotify and rawName and rawName ~= "" then
            pcall(function()
                Library.SendNotification({ 
                    title = "Target Player", 
...
-- Hook System.curve.get_cframe
task.spawn(function()
    while not (System and System.curve and typeof(System.curve.get_cframe) == "function") do
        task.wait(0.2)
    end

    if not getgenv()._Original_System_GetCFrame then
        getgenv()._Original_System_GetCFrame = System.curve.get_cframe
    end

    if not getgenv()._IsTargetParryHooked then
        getgenv()._IsTargetParryHooked = true
        System.curve.get_cframe = function(...)
            local targetCF = getgenv()._GetTargetParryCFrame()
            if targetCF then
                return targetCF
            end
            if getgenv()._Original_System_GetCFrame then
                return getgenv()._Original_System_GetCFrame(...)
            end
        end
    end
end)
...
-- Khởi tạo Module "Target Player" trong SpamTab (hoặc MainTab)
local target_player_module = (SpamTab or MainTab):create_module({
    title = "Target Player",
    flag = "Target_Player_Parry",
    description = "Locks parry trajectory onto a targeted player",
    section = "left",
    callback = function(state)
        getgenv().TargetParryEnabled = state
        if getgenv().TargetParryNotify then
            pcall(function()
                Library.SendNotification({
                    title = "Target Player",
                    text = state and "ON" or "OFF",
...
target_player_module:change_state(false)

target_player_module:create_textbox({
    title = "Target Username",
    flag = "Target_Username_Input",
...
local player_dropdown = target_player_module:create_dropdown({
    title = "Select Player List",
    flag = "Target_Player_Dropdown",
    options = getPlayerNames(),
    maximum_options = 100,
    callback = function(selectedName)
        setTargetPlayer(selectedName)
    end
})

target_player_module:create_button({
    title = "Clear Target",
    callback = function()
        getgenv().TargetUsername = ""
        getgenv()._LastTargetPlayer = nil
        if getgenv().TargetParryNotify then
            pcall(function()
                Library.SendNotification({
...
target_player_module:create_checkbox({
    title = "Notify",
    flag = "Target_Player_Parry_Notify",
    callback = function(value)
        getgenv().TargetParryNotify = value
    end
})

-- Tự động làm mới Dropdown danh sách người chơi
local function refreshDropdown()
    if player_dropdown then
        local names = getPlayerNames()
        if typeof(player_dropdown.set_options) == "function" then
            player_dropdown:set_options(names)
        elseif typeof(player_dropdown.refresh) == "function" then
            player_dropdown:refresh(names)
        end
    end
end

local function onPlayersChanged(player, isRemoving)
    if isRemoving and getgenv()._LastTargetPlayer == player then
        getgenv()._LastTargetPlayer = nil
        getgenv().TargetUsername = ""
    end
    refreshDropdown()
...
getgenv()._TargetParryConnections["PlayerAdded"] = Players.PlayerAdded:Connect(function(p) onPlayersChanged(p, false) end)
getgenv()._TargetParryConnections["PlayerRemoving"] = Players.PlayerRemoving:Connect(function(p) onPlayersChanged(p, true) end)
...
-- ============================================================
-- R6 FORCE MODULE (persistent — re-applies on respawn)
-- ============================================================
getgenv().ForceR6Enabled = getgenv().ForceR6Enabled or false
getgenv()._ForceR6State = getgenv()._ForceR6State or {
    connections = {},
    originalRigType = nil,
    applying = false,
}

local function _r6_cleanup_connections()
    for _, conn in pairs(getgenv()._ForceR6State.connections) do
        if typeof(conn) == "RBXScriptConnection" then
            pcall(function() conn:Disconnect() end)
        end
    end
    getgenv()._ForceR6State.connections = {}
end

local function _r6_apply_to_character(character)
    if not character or getgenv()._ForceR6State.applying then return end
    getgenv()._ForceR6State.applying = true

    local ok, err = pcall(function()
        local humanoid = character:WaitForChild("Humanoid", 5)
        if not humanoid then return end

        if getgenv()._ForceR6State.originalRigType == nil then
            getgenv()._ForceR6State.originalRigType = humanoid.RigType
        end

        if humanoid.RigType == Enum.HumanoidRigType.R6 then
            getgenv()._ForceR6State.applying = false
            return
        end
        ...
        local descOk = pcall(function()
            desc = PlayersSvc:GetHumanoidDescriptionFromUserId(lp.UserId)
        end)
    ...
    if not ok then
        warn("[R6 FORCE] apply error:", tostring(err))
    end
end

local function _r6_hook_respawn()
    _r6_cleanup_connections()
    local PlayersSvc = game:GetService("Players")
    local lp = PlayersSvc.LocalPlayer

    if lp.Character then
        task.spawn(_r6_apply_to_character, lp.Character)
    end

    getgenv()._ForceR6State.connections.charAdded = lp.CharacterAdded:Connect(function(char)
        task.wait(0.15)
        _r6_apply_to_character(char)
    end)

    getgenv()._ForceR6State.connections.charAppearance = lp.CharacterAppearanceLoaded:Connect(function(char)
        task.wait(0.1)
        _r6_apply_to_character(char)
    end)
end

local function _r6_disable()
    _r6_cleanup_connections()
    local PlayersSvc = game:GetService("Players")
    local lp = PlayersSvc.LocalPlayer
    if lp.Character and getgenv()._ForceR6State.originalRigType then
        pcall(function()
            local humanoid = lp.Character:FindFirstChildOfClass("Humanoid")
            if humanoid and humanoid.RigType ~= getgenv()._ForceR6State.originalRigType then
                lp:LoadCharacter()
            end
        end)
    end
    getgenv()._ForceR6State.originalRigType = nil
end

local force_r6_module = PlayerTab:create_module({
    title = "Force R6",
    description = "Force your character to R6 rig (persistent on respawn)",
    flag = "ForceR6Enabled",
    section = "left",
    callback = function(value: boolean)
        getgenv().ForceR6Enabled = value
        ...
        if value then
            ...
            pcall(function()
                Library:notify({
                    title = "R6 Force",
                    description = "Enabled — character will re-apply on respawn",
                    variant = "success",
                    duration = 3,
                })
            end)
